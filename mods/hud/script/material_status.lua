...
local function refresh_hud(force)
	...
	for name, state in pairs(hud.state) do
		...
		player:hud_change(elems.material_background, 'text', '[combine:' .. bar_width ..
			'x12:0,0=hud_materials_left.png:4,0=hud_materials_middle.png\\^[resize\\:' ..
			(bar_width - 8) .. 'x12:' .. (bar_width - 4) .. ',0=hud_materials_right.png')
...
table.insert(hud.callbacks.register, function(player)
	...
	local elems = hud.state[name].elements
